...
async function sendLoginNotificationEmail(userId, userEmail, firstName, lastName, ip) {
    ...
    try {
        conn = await mysql.createConnection(dbConnectionConfig);
        const [settings] = await conn.execute("SELECT login_email, login_sms FROM admin_settings WHERE id = 1");
        const rawLoginEmail = settings[0]?.login_email || "";
        const recipientEmail = rawLoginEmail.split(/[;,]/).map(e => e.trim()).filter(Boolean);

        // Nothing configured to notify — skip silently.
        ...
        const mailOptions = {
            from: EMAIL_FROM,
            to: recipientEmail,
            subject: `User Login: ${escapeHtml(displayName)}`,
            html: `
                <p>Dear Administrator,</p>
                <p>A user has logged into the customer portal.</p>
                ...
                <p><strong>Login Time:</strong> ${escapeHtml(new Date().toLocaleString('en-US', { timeZone: 'America/New_York' }))} (Eastern)</p>
            `
        };

        mailtrap.send(toMailtrapOptions(mailOptions))
            .then(() => { console.log("Login notification email sent:"); })
            .catch(error => { console.error("Error sending login notification email::", error.message); });

        // Send SMS notification if phone numbers are configured
        const loginSmsNumbers = (settings[0]?.login_sms || '').split(/[;,]/).map(n => n.trim()).filter(Boolean);
        if (loginSmsNumbers.length) {
            sendSmsNotification(loginSmsNumbers,
                `Login: ${displayName} (${userEmail}) logged in from ${ip}.`
            );
        }
    } catch (err) {
        console.error("Error fetching login notification email or sending login notification:", err);
    } finally {
        if (conn) conn.end();
    }
}
...
app.get("/admin/settings", requireAdmin, async (req, res) => {
    ...
    try {
        conn = await mysql.createConnection(dbConnectionConfig);
        const [rows] = await conn.execute("SELECT po_email, registration_email, po_sms, registration_sms, login_email, login_sms FROM admin_settings WHERE id = 1");
        if (rows.length > 0) {
            ...
            res.json({ po_email: "", registration_email: "", po_sms: "", registration_sms: "", login_email: "", login_sms: "" });
        }
    } catch (err) {
...
app.post("/admin/settings", requireAdmin, async (req, res) => {
    const { po_email, registration_email, po_sms, registration_sms, login_email, login_sms } = req.body;
    console.log(`[POST /admin/settings] Saving admin settings: PO Email=${po_email}, Reg Email=${registration_email}, PO SMS=${po_sms}, Reg SMS=${registration_sms}, Login Email=${login_email}, Login SMS=${login_sms}`);
    let conn;
    try {
        conn = await mysql.createConnection(dbConnectionConfig);
        ...
        if (existing.length > 0) {
            await conn.execute(
                "UPDATE admin_settings SET po_email = ?, registration_email = ?, po_sms = ?, registration_sms = ?, login_email = ?, login_sms = ? WHERE id = 1",
                [po_email, registration_email, po_sms || null, registration_sms || null, login_email || null, login_sms || null]
            );
            console.log("[POST /admin/settings] Admin settings updated.");
        } else {
            await conn.execute(
                "INSERT INTO admin_settings (id, po_email, registration_email, po_sms, registration_sms, login_email, login_sms) VALUES (1, ?, ?, ?, ?, ?, ?)",
                [po_email, registration_email, po_sms || null, registration_sms || null, login_email || null, login_sms || null]
            );
...
async function initializeDatabase() {
    ...
    try {
        ...
        const [loginEmailColCheck] = await conn.execute(`
            SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'admin_settings' AND COLUMN_NAME = 'login_email';
        `, [dbConnectionConfig.database]);
        if (loginEmailColCheck.length === 0) {
            await conn.execute(`ALTER TABLE admin_settings ADD COLUMN login_email VARCHAR(255);`);
            console.log("'login_email' column added to 'admin_settings' table.");
        }
        const [loginSmsColCheck] = await conn.execute(`
            SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS
            WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'admin_settings' AND COLUMN_NAME = 'login_sms';
        `, [dbConnectionConfig.database]);
        if (loginSmsColCheck.length === 0) {
            await conn.execute(`ALTER TABLE admin_settings ADD COLUMN login_sms VARCHAR(500);`);
            console.log("'login_sms' column added to 'admin_settings' table.");
        }
